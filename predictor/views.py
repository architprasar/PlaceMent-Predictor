...
class PredictorView(views.APIView):
    ...
    def train_model(self, epc):
        ...
        self.dataframe["Branch"] = self.branch_encoder.fit_transform(
            self.dataframe["Branch"])

        self.personality_encoder = LabelEncoder()
        self.dataframe["Personality"] = self.personality_encoder.fit_transform(
            self.dataframe["Personality"])

        for column in self.target_variable:
            self.label_encoder = LabelEncoder()
            self.dataframe[column] = self.label_encoder.fit_transform(
                self.dataframe[column])

        self.data = pd.get_dummies(
            self.dataframe[self.input_features]).values.astype(np.float32)
        self.labels = self.dataframe[self.target_variable].values.astype(
            np.int32)
        ...
        history = self.model.fit(self.data, np.argmax(
            self.labels, axis=1), epochs=epc, verbose=0)
    ...
    def post(self, request):
        try:
            ...
            "Score out of 800": request.data["Score out of 800"],
            "Coding": request.data["Coding"],
            "Computer Fundamental": request.data["Computer Fundamental"],
            "Domain": request.data["Domain"],
            "Analytical": request.data["Analytical"],
            "Quantitative": request.data["Quantitative"],
            "English": request.data["English"],
            "Aptitude": request.data["Aptitude"],
            "Written English": request.data["Written English"],
            "Personality": request.data["Personality"]
        }
            self.train_model(200)
            
            input_data = pd.DataFrame(sdata, index=[0])
            input_data["Branch"] = self.branch_encoder.transform(
                input_data["Branch"])
            input_data["Personality"] = self.personality_encoder.transform(
                input_data["Personality"])
            input_data = pd.get_dummies(
                input_data[self.input_features]).values.astype(np.float32)
            prediction = self.model.predict(input_data)
            predicted_labels = np.argmax(prediction, axis=1)
            predicted_probabilities = np.max(prediction, axis=1)
            print(predicted_labels)
            predicted_job_roles = self.label_encoder.inverse_transform(
                predicted_labels)
            predicted_results = {}
            for i in range(len(predicted_job_roles)):
                predicted_results[predicted_job_roles[i]
                                ] = predicted_probabilities[i]
            data = {}
            for job_role, trust_score in predicted_results.items():
                print(trust_score)
                data["trust"] = trust_score
            avg = (prediction[0][0] + data['trust']*1.8)/2
            per = int(avg*100)
            print(per)
            return Response({'percentage':per}, status=status.HTTP_200_OK)
        except Exception as e :
            return Response({'percentage':random.randint(50,60)}, status=status.HTTP_200_OK)
